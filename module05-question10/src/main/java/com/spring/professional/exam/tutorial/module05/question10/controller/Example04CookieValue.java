...
public class Example04CookieValue {
	...
	public String example04ReadCookie2(@CookieValue("user-id") Cookie cookie) {
		return String.format("Received Cookie name = [%s] and value = [%s] \n", cookie.getName(), cookie.getValue());
	}
	...
	@ResponseBody
	public String example04ReadCookie4(
			@CookieValue(value = "user-id", required = false, defaultValue = "N/A") String userId) {
		return String.format("user-id from  Cookie = [%s]  \n", userId);
	}
	...
	@ResponseBody
	public String example04ReadCookie5(
			@CookieValue(value = "user-id") Optional<String> userId) {
		return String.format("user-id from  Cookie = [%s]  \n", userId.orElse("N/A"));
	}
}
