package com.spring.professional.exam.tutorial.module05.question08.controller;
...
public class IndexController {
	...
	// curl "localhost:8080/actionC/countries/US/cities/NYC"
	...
	// curl "localhost:8080/actionD/countries/US/cities/NYC"
	...
	// curl "localhost:8080/actionE/countries/US/cities/NYC/zip/3456"
	@GetMapping("/actionE/countries/{country}/cities/{city}/zip/{code}")
	@ResponseBody
	public String actionE(@PathVariable Map<String,String> params) {
		String paramAsString = params.entrySet().stream()
								.map(entry -> String.format("[%s] -> [%s]", entry.getKey(),entry.getValue()))
								.collect(Collectors.joining(", "));
		return String.format("Retrieved parameter map - [%s]", paramAsString);
	}
	...
	{
		return String.format("Retrieved countries = [%s] \n", String.join(", ", countries));
	}
}
